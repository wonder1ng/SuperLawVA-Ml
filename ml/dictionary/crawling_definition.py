    ...
    while True:
        ...
        try:
            ...
            if not law_search_container:
                print(
                    "[1단계 오류] 'LsTrmSearch' 태그를 찾을 수 없습니다. 응답 구조를 확인해주세요."
                )
            ...
            if not terms_elements:
                print(
                    f"[1단계 완료] 페이지 {page_num}에 더 이상 용어가 없습니다. 메타데이터 수집 종료."
                )
                break  # 더 이상 데이터가 없으면 루프 종료

            for item_element in terms_elements:
                term_id = (
                    item_element.find("법령용어ID").get_text(strip=True)
                    if item_element.find("법령용어ID")
                    else "N/A_ID"
                )
                term_name = (
                    item_element.find("법령용어명").get_text(strip=True)
                    if item_element.find("법령용어명")
                    else "N/A_NAME"
                )

                all_term_metadata.append(
                    {"법령용어ID": term_id, "법령용어명": term_name}
                )

            print(
                f"[1단계 진행] {page_num}페이지 완료. 현재까지 누적 용어 메타데이터: {len(all_term_metadata)}개"
            )
            page_num += 1
...
def get_legal_term_definition_by_id(term_id):
    ...
    try:
        ...
        if ls_trm_service_container:
            definition = (
                ls_trm_service_container.find("법령용어정의").get_text(strip=True)
                if ls_trm_service_container.find("법령용어정의")
                else "정의 없음"
            )
            return definition
        else:
            print(
                f"[2단계 오류] ID {term_id}: 'LsTrmService' 태그를 찾을 수 없습니다. 응답 구조 확인 필요."
            )
            # print(response.text[:500]) # 디버깅용 응답 내용 출력
    ...
    if not term_metadata_list:
        ...
        for idx, term_meta in enumerate(term_metadata_list):
            ...
            if (idx + 1) % 50 == 0 or idx == len(term_metadata_list) - 1:
                print(
                    f"[2단계 수집 중] {idx+1}/{len(term_metadata_list)} 용어 처리 중: '{term_name}' (ID: {term_id})"
                )
            ...
            if (
                ...
                print(
                    f"[경고] 용어 '{term_name}' (ID: {term_id})의 정의를 가져오지 못했습니다. 스킵 또는 'N/A' 처리."
                )
                final_legal_terms_data.append(
                    {
                        "법령용어명": term_name,
