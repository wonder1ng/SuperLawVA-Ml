...
class DocumentSearchService:
    ...
    async def load_vectorstores(self):
        ...
        try:
            ...
            # 법령 벡터스토어 로드
            print("📚 법령 벡터스토어 로딩 중...")
            self.law_vectorstore = await loop.run_in_executor(
                None, self._load_law_with_debug
            )
            print("✅ 법령 벡터스토어 로드 완료")

            # 판례 벡터스토어 로드
            print("⚖️ 판례 벡터스토어 로딩 중...")
            self.case_vectorstore = await loop.run_in_executor(
                None, self._load_case_with_debug
            )
    ...
    async def search_documents(self, user_query: str) -> Tuple[List, List]:
        ...
        try:
            ...
            law_docs = await loop.run_in_executor(
                None,
                lambda: self.law_vectorstore.similarity_search(
                    user_query, k=LAW_SEARCH_LIMIT
                ),
            )
            ...
            case_docs_with_scores = await loop.run_in_executor(
                None,
                lambda: self.case_vectorstore.similarity_search_with_score(
                    user_query, k=CASE_SEARCH_LIMIT
                ),
            )
    ...
    async def search_laws_only(self, query: str, limit: int = 10) -> List:
        ...
        try:
            loop = asyncio.get_event_loop()
            law_docs = await loop.run_in_executor(
                None, lambda: self.law_vectorstore.similarity_search(query, k=limit)
            )
