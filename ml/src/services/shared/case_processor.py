...
class CaseProcessor:
    ...
    async def analyze_case_for_letter(
        self, case_doc, user_query: str, contract_summary: str
    ) -> CaseSummary:
        """내용증명용 판례 분석"""
    ...
    async def analyze_case_for_review(
        self, case_doc, problematic_clause: str
    ) -> Dict[str, Any]:
    ...
    async def generate_case_summaries_for_letter(
        self, case_docs: list, user_query: str, contract_summary: str
    ) -> List[CaseSummary]:
    ...
    async def generate_case_analyses_for_review(
        self, case_docs: list, problematic_clauses: List[str]
    ) -> List[Dict[str, Any]]:
        """계약서 검토용 판례 분석 생성"""
    ...
    def convert_to_case_basis(
        self, case_summaries: List[CaseSummary]
    ) -> List[CaseBasis]:
    ...
    async def generate_case_summaries(
        ...
        """기존 함수명 호환성 유지"""
        return await self.generate_case_summaries_for_letter(
            case_docs, user_query, contract_summary
        )

    async def _analyze_single_case(
        self, doc, user_query: str, contract_summary: str
    ) -> CaseSummary:
        """기존 함수명 호환성 유지"""
...
def convert_to_review_case_format(
