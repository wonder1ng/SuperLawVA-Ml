...
from services.schema.shared_schema import LegalBasis, LegalReference
...
class LegalProcessor:
        ...
        for law in all_laws:
            ...
            for quoted_law in quoted_laws:
                ...
                if quoted_law in law:
                    should_exclude = True
                    print(
                        f"[DEBUG] ✂️ 제외됨: '{law}' (인용된 법령: '{quoted_law}' 포함)"
                    )
                    break
        ...
        for match in matches:
            ...
            for i, pattern in enumerate(law_patterns):
                ...
                for law_match in law_matches:
                    ...
                    if (
                        len(law_name) > 3
                        and law_name not in invalid_words
                        and not any(word in law_name for word in invalid_words)
                    ):
    ...
    def find_law_document(self, reference: str, law_docs: list):
        ...
        patterns = [
            ...
            re.compile(
                r"「([^」]+)」\s*제\s*(\d+)\s*조(?:\s*제\s*(\d+)\s*항)?(?:\s*제\s*(\d+)\s*호)?",
                re.UNICODE,
            ),
            ...
            # 패턴 3: 기존 패턴 (백업용)
            re.compile(
                r"(?:「)?([^」]*?(법|시행령|시행규칙))(?:」)?\s*제\s*(\d+)\s*조(?:\s*제\s*(\d+)\s*항)?(?:\s*제\s*(\d+)\s*호)?",
                re.UNICODE,
            ),
        ]
        ...
        for i, pattern in enumerate(patterns):
            ...
            if match:
                ...
                print(
                    f"[DEBUG] 파싱 결과 - 법령: '{law_name}', 조: '{article}', 항: '{clause}', 호: '{item}'"
                )
                break
            else:
        ...
        for i, doc in enumerate(law_docs):
            ...
            print(
                f"[DEBUG] 문서 {i}: '{doc_law_name_normalized}' vs '{law_name_normalized}'"
            )
    ...
    async def generate_legal_explanations(
        self, referenced_laws: Set[str], law_docs: list
    ) -> List[LegalReference]:
        """법령 상세 설명 생성 (개선된 버전)"""
        ...
        for law_ref in referenced_laws:
            ...
            # 실제 법령ID 추출
            law_id = target_doc.metadata.get("법령ID", "") or target_doc.metadata.get(
                "law_id", ""
            )
            if not law_id:
                law_id = "0"
    ...
    def convert_to_legal_basis(
        self, legal_explanations: List[LegalReference]
    ) -> List[LegalBasis]:
...
def convert_to_legal_basis(
    legal_explanations: List[LegalReference],
) -> List[LegalBasis]:
...
def convert_to_review_format(
    legal_explanations: List[LegalReference], analysis_results: List[Dict]
) -> List[Dict]:
    """계약서 검토용 형식 변환"""
    review_results = []
